...
namespace ToolsApp.Areas.Admin.Controllers
{
    ...
    public class UserManagementController : BaseController
    {
        ...
        public ActionResult _GetList(string UsernameSearch, string EmailSearch)
        {
            ...
            var List = db_.AspNetUsers.Where(p =>
            (UsernameSearch == "" || UsernameSearch == null || p.UserName.ToUpper().Contains(UsernameSearch.ToUpper())) &&
            (EmailSearch == "" || EmailSearch == null || p.Email.ToUpper().Contains(EmailSearch.ToUpper())) &&
            (p.IsLocked == false || p.hieuluc == true) && p.AccoutType == "Admin").ToList();
        ...
        {
            var users = db_.AspNetUsers.Select(p => p.UserName).ToList();
            #region Load user from DKSH db
            var allUsers = db_.AspNetUsers.Where(p => p.hieuluc == true &&
        ...
        {
            var model = db_.AspNetUsers.FirstOrDefault(p => p.Id == Id);
        ...
        public JsonResult _EditFun(QL_UserViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var item = db_.AspNetUsers.FirstOrDefault(p => p.Id == model.Id);
                    if (item == null)
                    {
